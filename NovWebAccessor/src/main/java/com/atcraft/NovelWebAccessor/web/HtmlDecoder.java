...
//		content_fixed=content_fixed.replaceAll("&nbsp;"," ");
	...
	public static NovInfo extractBookInfo(Document doc)
	{
		...
		String charset = doc.select("meta").attr("charset");
		if (charset.equals(""))
		{
			String temp = null;
			for (Element e : meta)
		...
		return new NovInfo(bookName.html(), introduction.text(), LocalDateTime.parse(updateTime_str), writter_str,
				charset);
	}
	...
	public static LinkedList<URL> extractChapterList_Remote(URL url) throws IOException
	{
		...
		int dt_num = 0;//检测列表标题数量，如果==2则意味着有正文和最新章节两栏
